...
class ManticoreEVM(ManticoreBase):
    ...
    def _compile_through_crytic_compile(filename, contract_name, libraries, crytic_compile_args):
        ...
        try:
            ...
            # libraries =[(libs[0],"0x00000000000000000001")] 
            ...
            # print("initcode",bytecode.hex())
            # print("runcode",runtime.hex())
    ...
    def solidity_create_contract(
        ...
        while contract_names:
            ...
            try:
                ...
                print("init code",md._init_bytecode.hex())
    ...
    def create_contract(self, owner, balance=0, address=None, init=None, name=None, gas=None):
        ...
        expected_address = evm.EVMWorld.calculate_new_address(int(owner), nonce=0)
    ...
    def multi_tx_analysis(
        ...
        owner_account = self.create_account(balance=10 ** 20, name="owner")
        attacker_account = self.create_account(balance=10 ** 20, name="attacker")
        # Pretty print
        ...
        while (current_coverage < 100 or not tx_use_coverage) and not self.is_killed():
            try:
                ...
                if tx_send_ether:
                    ...
                    self.constrain(Operators.OR(value > 2*10 ** 18,value == 0))
                else:
                    value = 0
        ...
        while (current_coverage < 100 or not tx_use_coverage) and not self.is_killed():
            try:
                ...
                if tx_send_ether:
                    value = self.make_symbolic_value()
                    # self.constrain(Operators.OR(value > 2*10 ** 18,value == 0))
                    self.constrain(Operators.OR(value > 2*10 ** 18,value == 1*10 ** 18))
                    # value = 2*10 ** 20
                else:
                    value = 0
