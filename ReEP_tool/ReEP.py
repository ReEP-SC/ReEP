...
import json
from detector.print_pragma_version import get_solc
from detector.manti_reen1 import d_execute
import argparse
...
class ReEP:
    ...
    def run(self, args):
                ...
                if (task1_msgsender == True):
                    ...
                    if (task3_modifier == True):
                        if task2_payable== True :
                            _,is_bug,state_id = d_execute(filename,Contract_name,Function_name)
        ...
        os.system("rm -rf mcore*")
        os.system("rm -rf disassemble.txt")
